const iconX = `<svg class="icon-x" viewBox="0 0 25 24" fill="none" xmlns="http://www.w3.org/2000/svg">
...
const iconO = `<svg class="icon-o" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
...
function play(element){
    ...
    if(svg){
        console.log('ja tem elemento aqui')
    }else{
        ...
        setMirror(element.getAttribute("data-position"));
        checkWinner();
        setTurn();
    }
...
function setTurn(){
    ...
    if(turn === "x"){
        turn = "o";
        turnElement.innerHTML = iconO + '<h2>Turno</h2>';
 
    }else{
        turn = "x";
        turnElement.innerHTML = iconX + '<h2>Turno</h2>';   
    }
}
...
function setMirror(position){
    ...
    var position = position.split(",").map(Number);
...
function checkWinner(){
    ...
    if(gameMirror[1][1] != null){

        if( gameMirror[0][0] === gameMirror[1][1] &&
            gameMirror[0][0] === gameMirror[2][2] ||
            gameMirror[2][0] === gameMirror[1][1] &&
            gameMirror[2][0] === gameMirror[0][2]) {
                return turn;
            }
    }

    for(let i = 0; i < gameMirror.length; i++) {
        
        if(gameMirror[i][0]!= null){

            if(gameMirror[i][0] === gameMirror[i][1] &&
                gameMirror[i][0] === gameMirror[i][2]){
                    return turn;
            }
        }

        if(gameMirror[0][i]!= null){

            if(gameMirror[0][i] === gameMirror[1][i] &&
                gameMirror[0][i] === gameMirror[2][i]){
                    return turn;
                }
        }

        if(gameMirror[i].includes(null) === false){
            countDraw ++;
            if(countDraw === 3){
                return 'draw';
            }
